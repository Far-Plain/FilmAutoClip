...
async function main() {
    ...
    async fetch(url, options = {}) {
      ...
      if (options.method === "POST") {
        ...
        return {
          ok: true,
          status: 200,
          async json() { return { accepted: true, stats: { users, frames } }; }
        };
      }
      return {
        ok: true,
  ...
  assert.equal(elements.get("#communityUserCount").textContent, "8");
  ...
  console.log("Statistics client: anonymous identity, retry queue and global counters — OK");
}
